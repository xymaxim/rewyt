<script lang="ts">
 ...
 function getStepLabel(): string {
    if (stepMs >= 3600000) return `${parseFloat((stepMs / 3600000).toFixed(1))} h`;
 ...
 function stepTime(direction: 1 | -1) {
     ...
     const newTime =
         explorer.selectedTime + direction * stepMs;
...
<div class="selection-toolbar__row overflow-hidden">
    <div class="selection-toolbar__item bg-[var(--rewyt-selected-light)] transition-all! flex overflow-hidden grid duration-600 ease-in-out">
        <div class="flex flex-row items-center gap-1 transition-all overflow-hidden">
        <Button
            title="Jump back"
            variant="ghost"
            size="icon"
      ...
      <Expandable.Root trigger="click" class="flex h-full items-center transition-all overflow-hidden">
          {@const context = getContext('expandable')}
          <Expandable.Trigger class="flex h-8 {context.open ? 'w-9 -rotate-30' : 'min-w-9'} px-2 text-xs font-medium rounded-full border-none hover:bg-accent items-center justify-center bg-[var(--rewyt-selected-900)]!">
              {#if !context.open}
                  {getStepLabel()}
              {/if}
        </Expandable.Trigger>

        <Expandable.Content class="rounded-xl items-center flex h-8 px-2 transition-all overflow-hidden">
            <div class="flex gap-1 items-center">
            
                <div class="flex-1">
                    <input bind:value={stepInputValue}
                           class="h-8 w-16 text-sm bg-white/50 outline-none rounded-xl px-2"
                           onchange={applyStep}
                    />
            </div>
            
            <Select.Root type="single"
                         bind:value={stepUnit} class="border-none outline-none"
                         onValueChange={applyStep}
            >
                <Select.Trigger class="h-8! w-16 text-sm bg-white/50 rounded-xl border-none outline-none">
                {unitLabels[stepUnit]}
              </Select.Trigger>
              <Select.Content>
